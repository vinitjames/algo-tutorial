...
template <template <typename, typename> class TNode, typename TData,
          typename TMeta>
class BST {
 public:
 ...
 private:
  node_type *_max(node_type *root) {
    ...
    while (currRoot->r_child() != nullptr) currRoot = currRoot->r_child();
    return currRoot;
  }

  node_type *_min(node_type *root) {
    node_type *currRoot = root;
    while (currRoot->l_child() != nullptr) currRoot = currRoot->l_child();
    return currRoot;
  }
  ...
  void _bfTraversal(std::function<void(node_type *)> for_each) {
    ...
    while (!nodes.empty()) {
      ...
      nodes.pop();
      if (currNode->l_child() != nullptr) nodes.push(currNode->l_child());
      if (currNode->r_child() != nullptr) nodes.push(currNode->r_child());
      for_each(currNode);
    }
  }
